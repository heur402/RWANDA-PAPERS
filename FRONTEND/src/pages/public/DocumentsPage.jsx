...
import {
  ...
  GraduationCap, School, Wrench, BookOpen, ClipboardList,
  BookMarked, PenTool, Layers, FlaskConical, FileText,
} from 'lucide-react'
...
const categoryIcons = {
  'Primary School': School, 'Secondary School': GraduationCap, 'TVET': Wrench,
  'University': BookOpen, 'National Exams': ClipboardList, 'Notes': BookMarked,
  'Assignments': PenTool, 'Modules': Layers, 'Research Papers': FlaskConical,
}
const categoryColors = [
  'bg-blue-50 text-blue-600', 'bg-green-50 text-green-600', 'bg-purple-50 text-purple-600',
  'bg-orange-50 text-orange-600', 'bg-red-50 text-red-600', 'bg-cyan-50 text-cyan-600',
  'bg-pink-50 text-pink-600', 'bg-indigo-50 text-indigo-600', 'bg-yellow-50 text-yellow-600',
]
  ...
  return (
    <section className="mb-14">
      <div className="flex items-center justify-between mb-5">
        <button onClick={() => onFilterCategory(category._id)} className="flex items-center gap-3 group">
          <div className={`w-9 h-9 rounded-xl ${colorClass} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform`}>
            <Icon className="w-5 h-5" />
          </div>
          <div className="text-left">
            <h2 className="text-lg font-bold text-gray-900 group-hover:text-primary-600 transition-colors">{category.name}</h2>
            {!loading && (
              <p className="text-xs text-gray-400">
                {docs.length}{docs.length === 20 ? '+' : ''} document{docs.length !== 1 ? 's' : ''}
              </p>
            )}
          </div>
        </button>
        <button onClick={() => onFilterCategory(category._id)}
      ...
      {loading ? (
        ...
        <ScrollRow>
          {docs.map((doc) => (
            <div key={doc._id} data-card className="flex-shrink-0 snap-start w-[48vw] sm:w-52 md:w-56 lg:w-60">
...
const FilteredView = ({ documents, loading, error, pagination, onPageChange }) => {
...
const DocumentsPage = () => {
  ...
  const hasFilter = !!(searchQuery || categoryFilter || yearFilter || subjectFilter)
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      try {
        ...
        setDocuments(data.data); setPagination(data.pagination)
      } catch { setError('Failed to load documents. Please try again.') }
      finally  { setLoading(false) }
    }
    fetch()
  }, [currentPage, searchQuery, categoryFilter, yearFilter, subjectFilter, hasFilter])
  ...
  const handlePageChange = (page) => {
    const p = new URLSearchParams(searchParams); p.set('page', page)
    setSearchParams(p); window.scrollTo({ top: 0, behavior: 'smooth' })
  }

  const handleFilterCategory = (catId) => {
    const p = new URLSearchParams(); p.set('category', catId); p.set('page', '1')
    setSearchParams(p); window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {hasFilter ? (
        <FilteredView documents={documents} loading={loading} error={error}
